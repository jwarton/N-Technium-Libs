...
void ntTruss::init(){
	...
	for (int i = 0; i < nodeCnt; i++){
		...
		int v = i * DOF + 1;
		int w = i * DOF + 2;
		nodes.at(i)->Q(0) = Q(u, 0);
		nodes.at(i)->Q(1) = Q(v, 0);
		nodes.at(i)->Q(2) = Q(w, 0);
	}
	///////////////////////////////////////////////////////////////////////////
	///////////////////////////////// DEFINE AXIAL FORCE MATRIX
	for (int i = 0; i < elementCnt; i++){
		af::array q = af::constant(0, 6, 6);
		af::array f0 = elements.at(i)->get_p();
		af::array f1 = elements.at(i)->get_p();
		P(i, 0) = f0(0, 0);
		P(i, 1) = f1(1, 0);
	}
	...
	for (int i = 0; i < elementCnt; i++){
		...
		S(i, 0) = P(i, 1) / a;
		if (sMax < af::max<float>(S(i, 0))){  ///CONFIRM USE OF af::max and af::min
			sMax = af::max<float>(S(i, 0));   ///
			eMax = i;
		}
		if (sMin > af::min<float>(S(i, 0))){
			sMin = af::min<float>(S(i, 0));
			eMin = i;
		}
	}
}
