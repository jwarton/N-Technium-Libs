...
protected:
	...
	af::array Q = af::constant(0, 3, 1,f32);				//nodal displacement: (x,y,z)
	af::array R = af::constant(0, 3, 1, f32); 			//nodal rotation: (u,v,w)
	af::array F = af::constant(0, 3, 1, f32); 			//force applied at node
	af::array M = af::constant(0, 3, 1, f32); 			//moment applied at node
...
inline void ntNode::set_FC(Vec3 F){
	this->F(0) = F.x;
	this->F(1) = F.y;
	this->F(2) = F.z;
}
inline void ntNode::set_MC(Vec3 M){
	this->M(0) = M.x;
	this->M(1) = M.y;
	this->M(2) = M.z;
}
